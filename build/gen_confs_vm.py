...
'''
기존 Configuration generation을 qemu vm에 맞도록 변경
NUMA node : 사용 x
CLIENT => Client 수도 1
    - Core 수 : 1
    - Thread : 1
...
# server, client port 및 mac addr 정의
SERVER_MAC_ADDR = ['de:ad:1e:00:00:01', 'de:ad:1e:00:00:02', 'de:ad:1e:00:00:03']
CLIENT_MAC_ADDR = ['de:ad:1e:00:00:04', 'de:ad:1e:00:00:05', 'de:ad:1e:00:00:06']
SERVER_IP_ADDR = ['10.0.2.1', '10.0.2.2', '10.0.2.3']
CLIENT_IP_ADDR = ['10.0.2.4', '10.0.2.5', '10.0.2.6']
...
def server_addr(port_id):

    mac_addr = SERVER_MAC_ADDR[port_id]
    ip_addr = SERVER_IP_ADDR[port_id]

    return mac_addr, ip_addr

def client_addr(port_id):

    mac_addr = CLIENT_MAC_ADDR[port_id]
    ip_addr = CLIENT_IP_ADDR[port_id]
...
def main():
    ...
    vm_datasets = [
        (KEY_LEN, VAL_LEN, NUM_ITEM)
    ]

    f = open('conf_prepopulation_empty', 'w')
    p = PrePopulationConf('server')
    p.set(0,KEY_LEN, VAL_LEN)
    p.write(f)

    for dataset, (key_length, value_length, num_items) in enumerate(vm_datasets):
        assert key_length >= len('%x' % (num_items - 1))    # for hexadecimal key
        #num_partitions = 64
        num_partitions = NUM_PARTITION # 16
        ...
        for num_hot_items in (0, 1):
            for zipf in [('uniform', 0.)]:
                get_ratio = 0.5
        ...
        for num_hot_items in (0, 32):
            for zipf in (('uniform', 0.), ('skewed', 0.99), ('single', 99.)):
                for get_ratio in (0., 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99, 1.):s
                    concurrency_list.append(LB(num_hot_items, zipf, get_ratio))
        '''
        ...
        for concurrency in concurrency_list:
            for mth_threshold in mth_threshold_list:
                ...
                s = ServerConf('server')
                for port_id in range(NUM_PORT):
                    s.add_port(*server_addr(port_id))
                '''
        ...
        for zipf in [('uniform', 0.)]:
            ...
            for thread_id in range(NUM_THREAD):
                if isolated_server_numa_nodes:
                    w.add_thread(list(range(NUM_PORT)), s.server_name, 0, num_items, key_length, value_length, zipf_theta, get_ratio, put_ratio, increment_ratio, load_batch_size, num_operations, duration)
                else:
                    w.add_thread(list(range(NUM_PORT)), s.server_name, -1, num_items, key_length, value_length, zipf_theta, get_ratio, put_ratio, increment_ratio, load_batch_size, num_operations, duration)
            w.write(f)
             # trans operations
            zipf_theta = zipf[1]
            for get_ratio, put_ratio, increment_ratio in (
                ...
                for batch_size in (1, 2, 4):
                    ...
                    for thread_id in range(NUM_THREAD):
                        if isolated_server_numa_nodes:
                            w.add_thread(list(range(NUM_PORT)), s.server_name, 0, num_items, key_length, value_length, zipf_theta, get_ratio, put_ratio, increment_ratio, batch_size, num_operations, duration)
                        else:
                            w.add_thread(list(range(NUM_PORT)), s.server_name, -1, num_items, key_length, value_length, zipf_theta, get_ratio, put_ratio, increment_ratio, batch_size, num_operations, duration)
                    w.write(f)

        '''
        # workload config 파일 생성
